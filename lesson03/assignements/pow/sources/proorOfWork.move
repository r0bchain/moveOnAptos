module blockchain_rev_addr::proorOfWork {
    ...
    public entry fun initialize(account: &signer, amount: u64) {
        assert!(signer::address_of(account) == @blockchain_rev_addr, error::permission_denied(1));
    ...
    // Topup the Treasury by the contract owner.
    public entry fun topupTreasure(account: &signer, amount: u64) acquires Treasury {
        assert!(signer::address_of(account) == @blockchain_rev_addr, error::permission_denied(1));
        let coin = withdraw<AptosCoin>(account, amount);
        let treasury = borrow_global_mut<Treasury>(@blockchain_rev_addr);
        coin::merge(&mut treasury.balance, coin);  // Merge the new coins with existing balance

        // treasury.balance = coin
      
    }

    #[view]
    public fun get_treasury_balance(): u64 acquires Treasury {
        let treasury = borrow_global<Treasury>(@blockchain_rev_addr);
    ...
    public entry fun proof_of_easy_work(account: &signer, nonce: u64) acquires Treasury, Solved {
        ...
        let solved = borrow_global_mut<Solved>(@blockchain_rev_addr);
    ...
    public fun get_easy_solved_status(): option::Option<address> acquires Solved {
        let solved = borrow_global<Solved>(@blockchain_rev_addr);
        solved.easy
    }
